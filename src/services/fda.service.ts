...
class FDAService {
  ...
  async searchDrug(brandName: string): Promise<FDADrugResponse> {
    if (!brandName) {
      return { results: [], error: 'No product name provided' };
    }
  ...
  async getAdverseEvents(drugName: string): Promise<any> {
    if (!drugName) {
      return { results: [] };
    }

    const query = encodeURIComponent(`patient.drug.medicinalproduct:"${drugName}"`);
    const url = `${this.baseUrl}/event.json?api_key=${this.apiKey}&search=${query}&limit=100`;
  ...
  async getDrugLabeling(productNdc: string): Promise<FDADrugResponse> {
    if (!productNdc) {
      return { results: [], error: 'No NDC code provided' };
    }
